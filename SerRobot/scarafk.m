...
global CURRENT_Q
global ORBITS
global GRID_TOOL %Πλέγμα αρπάγης
global GRID_Z %Πλέγμα αρπάγης Z-axis

init_globals % Αρχικοποίηση global μεταβλητών

d=CURRENT_Q(1)+CURRENT_Q(2)+CURRENT_Q(4);

if GRASP == 1 %Η διαφορά του προσανατολισμού του κάθε αντικειμένου, με αυτόν της αρπάγης
...
while toc<T(end)
    ...
    for i=1:length(T)
        ...
        if T(i)>a
            ...
            %--------------------------------
            cc1=q1(i);%Μεταβλητές για subplot
            cc2=q2(i);%Μεταβλητές για subplot
            cc3=q3(i);%Μεταβλητές για subplot
            cc4=q4(i);%Μεταβλητές για subplot
            %--------------------------------
            break;
